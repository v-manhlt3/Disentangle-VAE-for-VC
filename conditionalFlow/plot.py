...
def emnist_plot_samples(model, n_rows, dims_to_sample=torch.arange(1280), temp=1):
    """
    ...
    style_sample = torch.zeros(n_rows, 1280)
    ...
    data = model(latent.view(-1, 1280), rev=True).detach().cpu().numpy().reshape(n_rows, 10, 16, 80)
    im = data.transpose(0, 2, 1, 3).reshape(n_rows*16, 10*80)
    librosa.display.specshow(data[0,0], x_axis='time', y_axis='mel', sr=16000)
...
def emnist_plot_variation_along_dims(model, dims_to_plot):
    ...
    for i, dim in enumerate(dims_to_plot):
        ...
        latent.detach_()
        data = model(latent.view(-1, 1280), rev=True).detach().cpu().numpy().reshape(10, n_cols, 16, 80)
        im = data.transpose(0, 2, 1, 3).reshape(10*16, n_cols*80)
        # images at +1 and -1 std
        im_p1 = im[:, 16*2:80*3]
        im_m1 = im[:, 16*6:80*7]
        # full image with spacing between the two parts
        # print(im.shape)
        # print(np.ones((10*64, 3)), np.abs(im_p1-im_m1).shape)
        # im = np.concatenate([im, np.ones((10*64, 3)), np.abs(im_p1-im_m1)], axis=1)
        plt.imshow(im, vmin=0, vmax=1)
        plt.xticks([])
        plt.yticks([])
